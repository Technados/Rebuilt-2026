# FRC Rebuilt 2026

The Technaods' 2026 FRC Rebuilt robot code. This README includes all of the port numbers, mechnical/electrical configuration notes, and limelight configurations.

## Port Numbers:

**PDH:** CAN ID 9

### Drive Subsystem

**Drive Motors (4)**

  - **Left Front:** CAN ID 1, PDP 10
  
  - **Right Front:** CAN ID 2, PDP 9
  
  - **Left Rear:** CAN ID 3, PDP 12
  
  - **Right Rear:** CAN ID 4, PDP 6
  
  
**Steering Motors (4)**

  - **Left Front:** CAN ID 5, PDP 11
  
  - **Right Front:** CAN ID 6, PDP 8
  
  - **Left Rear:** CAND ID 7, PDP 13
  
  - **Right Rear:** CAN ID 8, PDP 5

### Intake Subsystem

**Intake Motor:** CAN ID 10, PDP [PORT]

**Pivot Intake:** CAND ID 11, PDP 7

### Feeder Subsystem

**Feeder Motor:** CAN ID 12, PDP 14

**Pre Shooter Motor:** CAN ID 13, PDP 15

### Shooter Subsystem

**Left Shooter Motor:** CAN ID 14, PDP 16

**Right Shooter Motor:** CAN 15, PDP 17

**Center Shooter Motor:** CAN 16, PDP [PORT]

## Mechnical/Electrical Configuration Notes:

### Intake Subsystem

**Pivot Motor:** 52.89

**Intake:** 1.5

### Feeder Subsystem

**Feeder** 6

## Limelight Configuration:

### Front

**Name:** limelight-front

### Back

**Name:** limelight-back
